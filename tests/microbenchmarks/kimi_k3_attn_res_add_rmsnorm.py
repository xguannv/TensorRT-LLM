...
def _parse_candidates(value: str) -> list[int]:
    ...
    if any(candidate not in {*range(1, 10), 12} for candidate in candidates):
        raise argparse.ArgumentTypeError(
            "candidate counts must be in [1, 9] or equal to 12")
    return candidates
...
def _make_inputs(num_candidates: int) -> CaseInputs:
    ...
    prefix_sum = torch.empty(
        shape, dtype=torch.bfloat16, device=device).uniform_(-0.05, 0.05)
...
def _three_kernel(inputs: CaseInputs) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    output = flashinfer_rmsnorm(
        mixed, inputs.output_rms_weight, RMS_EPS)
...
def _profile_case(
    ...
    for name, fn in modes:
        range_name = (
            f"attn_res_add|T=1|N={num_candidates}|mode={name}")
...
def main() -> None:
    ...
    for num_candidates in args.candidates:
        if args.profile:
            _profile_case(
                num_candidates, min(args.iterations, 100))
        else:
            print(
                json.dumps(
                    _benchmark_case(
