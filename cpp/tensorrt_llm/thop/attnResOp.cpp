...
std::tuple<at::Tensor, at::Tensor> attn_res_add_rmsnorm_fwd(at::Tensor layer_residual,
    at::Tensor layer_residual_add, at::Tensor block_residual, at::Tensor res_weight, at::Tensor rms_weight,
    at::Tensor output_rms_weight, double rms_eps, double output_rms_eps)
{
    TORCH_CHECK(layer_residual.dim() == 3, "attn_res_add_rmsnorm_fwd: layer_residual must be [T, B, H]");
    TORCH_CHECK(
        layer_residual_add.sizes() == layer_residual.sizes(),
    ...
    TORCH_CHECK((N >= 1 && N <= 9) || N == 12,
        "attn_res_add_rmsnorm_fwd: supported N values are [1, 9] and 12");

    TORCH_CHECK(layer_residual.scalar_type() == at::kBFloat16,
        "attn_res_add_rmsnorm_fwd: layer_residual must be bf16");
    TORCH_CHECK(layer_residual_add.scalar_type() == at::kBFloat16,
        "attn_res_add_rmsnorm_fwd: layer_residual_add must be bf16");
    TORCH_CHECK(block_residual.scalar_type() == at::kBFloat16,
        "attn_res_add_rmsnorm_fwd: block_residual must be bf16");
    TORCH_CHECK(res_weight.scalar_type() == at::kBFloat16,
        "attn_res_add_rmsnorm_fwd: res_weight must be bf16");
    TORCH_CHECK(rms_weight.scalar_type() == at::kBFloat16,
        "attn_res_add_rmsnorm_fwd: rms_weight must be bf16");
    TORCH_CHECK(output_rms_weight.scalar_type() == at::kBFloat16,
        "attn_res_add_rmsnorm_fwd: output_rms_weight must be bf16");
    TORCH_CHECK(layer_residual.is_contiguous() && layer_residual_add.is_contiguous()
    ...
    TORCH_CHECK(
        output_rms_weight.numel() == H, "attn_res_add_rmsnorm_fwd: output_rms_weight must have H elements");
