...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def _format_kv_cache_pool_group_sizes(self) -> list[str]:
        ...
        ``pool_group_id`` is the index the active backend assigns, and the C++
        and Python backends have been observed to order the same configuration's
        groups differently. Identify a group by its ``slot_size``, not by its
        id, when comparing logs from two runs.
        """
        try:
